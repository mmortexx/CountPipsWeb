  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-2.5 whitespace-nowrap text-secondary text-sm max-w-[220px] truncate">
        {nombreSetup(trade.setup, langDatos(lang))}
      </td>
      ...
      {/* Col 8 — R chip (pill). */}
      <td className="px-3 py-2.5 whitespace-nowrap text-right">
        <RChip value={trade.rMultiple} lang={langDatos(lang)} />
      </td>
  ...
  const filtered = useMemo(() => {
    ...
    return allTrades.filter((tr) => {
      ...
      if (q) {
        ...
        const hay =
          `${tr.instrument} ${tr.setup} ${nombreSetup(tr.setup, langDatos(lang))} ${tr.entryNote} ${tr.closeNote}`.toLowerCase();
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {SETUP_NAMES.map((s) => (
                <FilterChip
                  ...
                  group="setup"
                  label={nombreSetup(s, langDatos(lang))}
                >
